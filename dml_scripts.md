...
Планировщик запросов в Postgres сам решает, использовать ему индекс в этом запросе или не использовать. Так как в таблице с товарами и описаниями мало записей, планировщик решил что ему проще просканировать всю таблицу, чем использовать созданный индекс.
Не совсем понял принцип работы полнотекстового поиска. Поэтому задам вопросы на предстоящем семинаре.
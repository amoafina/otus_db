...
#### При INNER JOIN порядок соединений не имеет значение, из-за "умной" работы планировщика, он сам определяет, как лучше присоединить таблицы.
#### При LEFT JOIN порядок соединений имеет значение. Разный порядок присоединения может выдать разный результат, так как запрос выдает все строки из левой таблицы + строки с правой, удовлетворяющие условию в ON.
